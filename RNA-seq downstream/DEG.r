...
deg<-function(){
    ...
    ph<-pheatmap(deg_fpkm_log2+1,show_rownames = F,
         color = colorRampPalette(c("skyblue","white","red"))(256),
         treeheight_col = 15, treeheight_row=25,
         width = 30,height = 100)
    ggsave(paste0(output_dir,'/Expression','/fpkm_pheatmap.pdf'),ph,width = 6,height = 8)
    cat("All analyze end\n")
}
summary_plot<-function(){
        ...
        pp<-ggplot(vol_plot, aes(
          ...
          geom_text_repel(
            #添加差异标签
            data = vol_plot[vol_plot$pvalue < 0.05 & abs(vol_plot$log2FoldChange) >4,],
            aes(label = Gene),
            size = 3,
            segment.color = "black",
            show.legend = FALSE
